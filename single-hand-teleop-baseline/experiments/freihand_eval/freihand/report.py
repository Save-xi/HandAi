...
def render_eval_report(metrics: Dict[str, Any], *, split: str, prediction_path: str) -> str:
    ...
    lines = [
        ...
        f"| 平均 latency | {format_number(latency.get('mean'), 3, ' ms')} |",
        f"| P95 latency | {format_number(latency.get('p95'), 3, ' ms')} |",
        "",
        "## PCK 细分",
        "",
        "| 类型 | 阈值 | 数值 |",
...
def render_ppt_table(metrics: Dict[str, Any]) -> str:
    ...
    lines = [
        ...
        f"| 平均 latency_ms | {format_number(latency.get('mean'), 3, ' ms')} | 单帧平均处理时间 |",
        f"| P95 latency_ms | {format_number(latency.get('p95'), 3, ' ms')} | 95% 帧不超过该耗时 |",
    ]
    return "\n".join(lines) + "\n"
